/*
	SQL Server 2019 Compatibility Fix
	Removes columns from query_store_plan table that don't exist in SQL Server 2019
*/

USE QueryVaultDB;
GO

PRINT 'Fixing SQL Server 2019 compatibility for query_store_plan tables...';
GO

-- Drop and recreate the main table without SQL 2022+ columns
IF OBJECT_ID('dbo.query_store_plan', 'U') IS NOT NULL
	DROP TABLE dbo.query_store_plan;
GO

CREATE TABLE dbo.query_store_plan
(
	RunID INT NOT NULL,
	...
	-- Removed SQL 2022+ columns:
	-- has_compile_replay_script
	-- is_optimized_plan_forcing_disabled
	-- plan_type
	-- plan_type_desc

	CONSTRAINT PK_query_store_plan PRIMARY KEY NONCLUSTERED (RunID, plan_id)
) ON PS_RunID(RunID);
GO

CREATE CLUSTERED COLUMNSTORE INDEX CCI_query_store_plan 
	ON dbo.query_store_plan
	WITH (COMPRESSION_DELAY = 0 MINUTES)
	ON PS_RunID(RunID);
GO

-- Drop and recreate the maintenance table
IF OBJECT_ID('dbo.query_store_plan_PartitionMaintenance', 'U') IS NOT NULL
	DROP TABLE dbo.query_store_plan_PartitionMaintenance;
GO
...
PRINT 'Tables recreated for SQL Server 2019 compatibility';
GO

-- Update the stored procedure to match SQL 2019 columns
PRINT 'Updating usp_ArchiveQueryStore for SQL 2019...';
GO
