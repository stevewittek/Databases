/* Partition-maintenance mirror for one complete archived wait observation at Microsoft's documented grain. */

CREATE TABLE dbo.query_store_wait_stats_canonical_PartitionMaintenance
(
	RunID INT NOT NULL,
	...
	CONSTRAINT PK_query_store_wait_stats_canonical_PartitionMaintenance
		PRIMARY KEY NONCLUSTERED
		(RunID, plan_id, runtime_stats_interval_id, execution_type, wait_category),
	CONSTRAINT CK_query_store_wait_stats_canonical_PartitionMaintenance_stdev
		CHECK (stdev_aggregation_status IN
			(N'NATIVE_SINGLE_CONTRIBUTOR', N'UNAVAILABLE_MULTIPLE_CONTRIBUTORS')),
	CONSTRAINT CK_query_store_wait_stats_canonical_PartitionMaintenance_state
		CHECK (observation_state IN (N'COMPLETED', N'PROVISIONAL'))
) ON PS_RunID(RunID);
GO
CREATE CLUSTERED COLUMNSTORE INDEX CCI_query_store_wait_stats_canonical_PartitionMaintenance
	ON dbo.query_store_wait_stats_canonical_PartitionMaintenance
