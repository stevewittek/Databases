/* Partition-maintenance mirror for native Query Store wait rows retained as contributor lineage. */

CREATE TABLE dbo.query_store_wait_stats_contributor_PartitionMaintenance
(
	RunID INT NOT NULL,
	...
	CONSTRAINT PK_query_store_wait_stats_contributor_PartitionMaintenance
		PRIMARY KEY NONCLUSTERED (RunID, wait_stats_contributor_id)
) ON PS_RunID(RunID);
GO
CREATE CLUSTERED COLUMNSTORE INDEX CCI_query_store_wait_stats_contributor_PartitionMaintenance
	ON dbo.query_store_wait_stats_contributor_PartitionMaintenance
